...
UA='arbejdsstyrke-dashboard/2.1'
...
def fetch_csv(table,selections):
    ...
    if not rows:raise RuntimeError(f'{table} returnerede ingen rækker')
    return rows
def num(v):
...
def col(rows,words,fallback=None):
    cols=list(rows[0])
    for c in cols:
        if any(norm(w) in norm(c) for w in words):return c
    if fallback is not None:return cols[fallback]
    raise RuntimeError(f'Kolonne ikke fundet for {words}. Kolonner: {cols}')
def time_col(rows):
    cols=list(rows[0]);vc=value_col(rows)
    for c in cols:
        if c!=vc and any(x in norm(c) for x in ('tid','år','maaned','måned','kvartal')):return c
    return cols[-2] if len(cols)>=2 else cols[0]

def aku_current():
    meta=info('AKU100K');status=variable(meta,['beskæftigelsesstatus']);time=variable(meta,['kvartal','tid'])
    rows=fetch_csv('AKU100K',{status['id']:['*'],time['id']:['*']});sc=col(rows,['beskæftigelsesstatus'],0);tc=time_col(rows);vc=value_col(rows);g={}
...
def ras():
    ...
    focus=[]
    for a in ages:
        if any(x in norm(a) for x in ('55','60','65')):focus.append({'label':a,'values':[emp(y,a) for y in years]})
...
def employees():
    ...
    rows=fetch_csv('LBESK104',{sector['id']:[total],time['id']:['*']});tc=time_col(rows);vc=value_col(rows);labels=[r[tc] for r in rows];values=[num(r[vc]) for r in rows];order=sorted(range(len(labels)),key=lambda i:period_key(labels[i]));labels=[labels[i] for i in order];values=[values[i] for i in order]
...
def seniors():
    tables=ji.get('tables',{'format':'json'});table=ji.find_table(tables,['arbejdsmarkedsstatus for seniorer']);tid=str(table['table_id']);spec=ji.get(f'table/{tid}',{'format':'json'});geo=ji.find_hierarchy(spec,['område','geografi','kommune'],('_hele_landet','_nykom'));age=ji.find_hierarchy(spec,['alder']);status=ji.find_hierarchy(spec,['arbejdsmarkedsstatus','status']);alevel=senior_level(age)
    ...
    rows=ji.records(ji.get(f'data/{tid}',params));agec=ji.best_col(rows,['alder'],distinct=True);statusc=ji.best_col(rows,['arbejdsmarkedsstatus','status'],distinct=True);pctc=ji.best_col(rows,['procent'],exclude=['grad']);pc=ji.best_col(rows,['periode']);period=max((str(r.get(pc)) for r in rows),key=period_key);g={}
    for r in rows:
        st=ji.norm(r.get(statusc));a=str(r.get(agec) or '').strip();v=ji.num(r.get(pctc))
        if str(r.get(pc))==period and a and v is not None and ('loenmodtagerbeskaeftigelse i alt' in st or st=='loenmodtagerbeskaeftigelse'):g[a]=float(v)*100 if abs(float(v))<=1 else float(v)
    items=[]
    for a,v in g.items():
...
def main():
    m_aku,aku=aku_current();m_ras,rasd=ras();m_emp,empl=employees();m_proj,proj=projection();now=datetime.now(ZoneInfo('Europe/Copenhagen')).isoformat(timespec='seconds');failed=[]
    sources={'AKU100K':src(m_aku,'AKU100K',aku['labels'][-1],'1.000 personer','sæsonkorrigeret'),'RAS200':src(m_ras,'RAS200',rasd['period'],'pct.','ikke relevant'),'LBESK104':src(m_emp,'LBESK104',empl['labels'][-1],'personer','sæsonkorrigeret'),'FRDK126':src(m_proj,'FRDK126',proj['labels'][-1],'personer','ikke relevant')}
    sen={'period':None,'items':[]}
    try:
        sid,sen=seniors();sources['seniorer']={'state':'ok','source':'Jobindsats.dk / STAR','dataset':sid,'latestPeriod':sen['period'],'unit':'pct.','seasonalAdjustment':'ikke relevant'}
    except Exception as exc:
        failed.append(f'seniorer: {exc}');sources['seniorer']={'state':'missing','source':'Jobindsats.dk / STAR','dataset':'Arbejdsmarkedsstatus for seniorer','latestPeriod':None,'unit':'pct.','seasonalAdjustment':'ikke relevant','error':str(exc)}
    state='ok' if not failed else 'partial';data={'meta':{'checkedAt':now,'sourceStatus':sources,'updateStatus':{'state':state,'successful':[k for k,v in sources.items() if v['state']=='ok'],'failed':failed,'checkedAt':now}},'sections':{'aku':aku,'ras':rasd,'employees':empl,'projection':proj,'seniors':sen}}
    OUT.parent.mkdir(parents=True,exist_ok=True);OUT.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print('RAS',rasd['period'],'AKU',aku['labels'][-1],'status',state)
