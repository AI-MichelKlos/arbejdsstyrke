...
def get(path,params=None):
    ...
    if not token: raise RuntimeError('JOBINDSATS_API_TOKEN mangler. GitHub-secretet API_ADGANG skal være tilgængeligt i repositoryet.')
    url=f"{API_ROOT}/{path.lstrip('/')}"
    if params:url+='?'+urllib.parse.urlencode(params,safe=':,*')
    req=urllib.request.Request(url,headers={'Accept':'application/json','Authorization':f'Bearer {token}','User-Agent':'arbejdsstyrke-dashboard/2.0'})
    last=None
...
def find_table(payload,phrases):
    ...
    if not scored:raise RuntimeError('Ingen Jobindsats-tabeller fundet.')
    d,b=scored[0][2],scored[0][3]
    if not all(all(w in b for w in p.split() if len(w)>3) for p in wanted):
        raise RuntimeError(f'Kunne ikke identificere Jobindsats-målingen sikkert. Bedste kandidat: {d.get("table_id")} {b[:220]}')
    return d
